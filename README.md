...
Run unit tests for state management and API logic:
...
Unit tests cover:
- `tests/unit/state.test.ts` - cabana booking state management (`isCabanaBooked`, `bookCabana`, `getBooking`)
- `tests/unit/api.test.ts` - API data validation (map parsing, guest validation)

### E2E Tests (Playwright)

Run end-to-end tests for the full booking flow:

```bash
npm run test:e2e
```

**Prerequisites:** The dev server must be running. Playwright will automatically start it if not already running.

E2E tests cover:
- `tests/e2e/booking.spec.ts` - Complete booking flow: click cabana, fill form, submit, verify confirmation
- `tests/e2e/map.spec.ts` - Map rendering, legend display, tile types
...
- `vitest.config.ts` - Unit test configuration (environment: node)
- `playwright.config.ts` - E2E test configuration (base URL: http://localhost:5173)
